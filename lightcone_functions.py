...
def LOAD_DATA( DATABASE='150Mpc_r25', 
    ...
    ### LC SLICE, 100x2200, 10 slices per LC
        ...
        file_name = '/amphora/nicolas.gillet/LC_data/LC_SLICE10_px100-2200_N10000_randICs.npy'
        Nsimu = 10000 ### number of LC
        BoxInLC = 11  ### number of concatenate boxes/LC
...
class DataGeneratorOLD(keras.utils.Sequence):
    ...
    def __init__(self, 
        ...
        self.n_pre_load = 10000
        self.count_batch = 0
        self.count_preload = 0
        self.pre_loaded_data = pre_loaded_data
        
        ### shuffle the indexes
        self.on_epoch_end()
        
    def pre_load_NData( self ):
        
        if self.pre_loaded_data:
            t_in = time.time()
            print( "PRELOAD DATA" )
            # Initialization
            X = np.empty((self.n_pre_load, *self.dim, self.n_channels), dtype=np.float32)
            y = np.empty((self.n_pre_load, self.n_params), dtype=np.float32)

            for i, ID in enumerate( self.indexes[self.count_preload*self.n_pre_load:(self.count_preload+1)*self.n_pre_load] ):
            ...
            self.count_preload += 1
            t_out = time.time()
            print( 'Time to load the data: ', t_out-t_in, ' s, ', (t_out-t_in)/60., ' min' )
        else:
            pass
    ...
    def __getitem__(self, index):
        ...
        if not(self.pre_loaded_data):
            ...
            X = self.X_preload[ self.count_batch*self.batch_size:(self.count_batch+1)*self.batch_size ]
            y = self.y_preload[ self.count_batch*self.batch_size:(self.count_batch+1)*self.batch_size ]
            
            self.count_batch +=1
            if self.count_batch == (self.n_pre_load / self.batch_size ):
                self.pre_load_NData()
                self.count_batch = 0
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, 
        ...
        self.DATA_DIR = DATA_DIR
        self.HDF5 = HDF5
        self.which_set = which_set ### for HDF5, train, validation or test
                
        self.n_pre_load = 80000 ### HARDCODED HERE FOR THE MOMENT
        self.count_batch = 0
        self.count_preload = 0
        self.pre_loaded_data = pre_loaded_data
        
        ### TO INITIALIZE THE RANDOMIZE
        self.on_epoch_end()
        
    def pre_load_NData( self ):
        """ pre-load some data in memory, only for training for the moment """
        ...
        if self.pre_loaded_data:
            ...
            if self.HDF5:
                ### The IDs have to be sorted to access HDF5 files 
                #list_IDs_preload = self.indexes[ self.count_preload*self.n_pre_load:
                #                                 (self.count_preload+1)*self.n_pre_load]
                
                #ID_sorted_list_IDs_preload = np.argsort( list_IDs_preload )
                #sorted_list_IDs_preload = list_IDs_preload[ID_sorted_list_IDs_preload]
                #ID_original_order_list_IDs_preload = np.argsort(ID_sorted_list_IDs_preload)
                ### The IDs have to be in list
                #with h5py.File( self.DATA_DIR, 'r') as f:
                #    X = f['LC_train'][ list(sorted_list_IDs_preload) ][ list(ID_original_order_list_IDs_preload) ]
                #    y = f['Param_train'][ list(sorted_list_IDs_preload) ][ list(ID_original_order_list_IDs_preload) ]
                list_IDs_preload = np.arange( self.count_preload*self.n_pre_load,
                                             (self.count_preload+1)*self.n_pre_load )
                #print( "LAST ID : ", list_IDs_preload[-1] )
                ### The IDs have to be in list
                with h5py.File( self.DATA_DIR, 'r', swmr=True) as f:
                    X = f['LC_train'][ list(list_IDs_preload) ]
                    y = f['Param_train'][ list(list_IDs_preload) ]
                if self.shuffle == True:
                    #print("I AM HERE")
                    shuffle_index = np.arange( self.n_pre_load ) ### ordered indices
                    np.random.shuffle( shuffle_index )
                    X = X[shuffle_index]
                    y = y[shuffle_index]
            ...
            self.count_preload += 1
            t_out = time.time()
            print( '\n Time to load the data: ', t_out-t_in, ' s, ', (t_out-t_in)/60., ' min' )
        else:
            pass
    ...
    def __getitem__(self, index):
        ...
        if not(self.pre_loaded_data) or not(self.which_set=='train'):
            ...
            list_IDs_temp = [self.list_IDs[k] for k in indexes]
            # Generate data
            X, y = self.__data_generation( np.array( list_IDs_temp, dtype=np.int32 ) )
            #X, y = self.__data_generation( np.array( indexes, dtype=np.int32 ) )
        
        else:
            #X = self.X_preload[ self.count_batch*self.batch_size:(self.count_batch+1)*self.batch_size ]
            #y = self.y_preload[ self.count_batch*self.batch_size:(self.count_batch+1)*self.batch_size ]
            
            index_tmp = np.int( index%(self.n_pre_load / self.batch_size ) )
            
            X = self.X_preload[ index_tmp*self.batch_size:(index_tmp+1)*self.batch_size ]
            y = self.y_preload[ index_tmp*self.batch_size:(index_tmp+1)*self.batch_size ]
            
            #print( "INDEX : ", index, ", INDEXMOD : ", index_tmp, ", COUNT : ", self.count_batch )
            
            self.count_batch +=1
            #if self.count_batch == (self.n_pre_load / self.batch_size ):
            if not( index_tmp ) and index: ### and index : is to avoid a reload at the begening
                self.pre_load_NData()
                self.count_batch = 0
        return X, y
        #return [X,X], [y,y]
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        if self.HDF5:
            ### The IDs have to be sorted to access HDF5 files 
            ID_sorted_list_IDs_temp = np.argsort( list_IDs_temp )
            sorted_list_IDs_temp = list_IDs_temp[ID_sorted_list_IDs_temp]
            ID_original_order_list_IDs_temp = np.argsort(ID_sorted_list_IDs_temp)
            ### The IDs have to be in list
            with h5py.File( self.DATA_DIR, 'r', swmr=True) as f:
                if self.which_set=='train':
                    X = f['LC_train'][ list(sorted_list_IDs_temp) ][list(ID_original_order_list_IDs_temp)]
                    y = f['Param_train'][ list(sorted_list_IDs_temp) ][list(ID_original_order_list_IDs_temp)]
                if self.which_set=='test':
                    X = f['LC_test'][ list(sorted_list_IDs_temp) ][list(ID_original_order_list_IDs_temp)]
                    y = f['Param_test'][ list(sorted_list_IDs_temp) ][list(ID_original_order_list_IDs_temp)]
                if self.which_set=='validation':
                    X = f['LC_val'][ list(sorted_list_IDs_temp) ][list(ID_original_order_list_IDs_temp)]
                    y = f['Param_val'][ list(sorted_list_IDs_temp) ][list(ID_original_order_list_IDs_temp)]
        else:
            # Initialization
            X = np.empty((self.batch_size, *self.dim, self.n_channels), dtype=np.float32)
            y = np.empty((self.batch_size, self.n_params), dtype=np.float32)
